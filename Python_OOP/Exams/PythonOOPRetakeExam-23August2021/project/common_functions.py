...
def check_lists(function):
    def check(**kwargs):
        ...
        ob = [value for value in kwargs["collection"] if function(getattr(value, att, name),name,op=operator) and function(getattr(value,attr1,name1),name1,op=operator2)]
        if condition:
            condition = bool(ob)
        else:
...
def not_in_collection_or_error(*args,**kwargs):
    """\npossible positional parameters "collection","att","name","message","exc","error_raise_condition" \n see check_collections for details"""
    kwargs = args_to_kwargs(*args, **kwargs)
    return check_collections(**kwargs,error_raise_condition=False)
