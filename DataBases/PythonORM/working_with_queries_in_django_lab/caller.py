...
def find_authors_nationalities():
    ...
    return '\n'.join(f'{author} is {author.nationality}' for author in authors)

def order_books_by_year():
    return '\n'.join(
        f"{book.publication_year} year: {book.title} by {book.author}"
        for book in
        Book.objects.all().order_by('publication_year','title')
    )


def delete_review_by_id(id):
    review = Review.objects.get(pk=id)
    res = f'Review by {review.reviewer_name} was deleted'
    review.delete()
    return res
...
def filter_authors_by_nationalities(nationality):
    return '\n'.join(
        (author.biography or str(author))
        for author in
...
def filter_authors_by_birth_year(year_one,year_two):
    return '\n'.join(
        ...
        for author in
        Author.objects.filter(birth_date__year__range=(year_one,year_two)).order_by('-birth_date')
    )
